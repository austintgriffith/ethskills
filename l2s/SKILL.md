...
**MegaETH is live.** Launched mainnet February 9, 2026. An optimistic rollup (OP Stack + Kailua proofs) with 10ms mini-blocks, `eth_sendRawTransactionSync` (EIP-7966) for instant receipts, and a multidimensional gas model. Uses EigenDA for data availability. Chain ID 4326.
...
| **MegaETH**       | Optimistic (OP Stack) | <$0.001       | 10ms (mini) / 1s (EVM) | ~10.5 days | 4326     |
...
| Real-time / streaming apps   | **MegaETH**                           | 10ms mini-blocks, instant receipts via EIP-7966, sub-cent fees |
...
- **Launched:** February 9, 2026 (mainnet). Chain ID 4326.
- **Type:** Optimistic rollup (OP Stack + Kailua proof system). EigenDA for data availability.
- **Key innovation: 10ms mini-blocks + instant receipts**
  - `eth_sendRawTransactionSync` (EIP-7966) — returns receipt in <10ms, no polling needed
  - Mini-block subscriptions via WebSocket for real-time streaming data
  - Sustained 35,000 TPS in stress tests (10.7B txs processed)
- **Gas model:** Multidimensional — separates compute and storage costs. Base fee stable at 0.001 gwei (no EIP-1559 adjustment). Skip `eth_maxPriorityFeePerGas` (returns 0). SSTORE 0→non-zero costs 2M gas x multiplier — design for slot reuse, not constant allocation.
- **Block gas limit:** 10 billion (10^10) gas per EVM block
- **Contract limits:** 512 KB (vs Ethereum's 24 KB)
- **WebSocket:** Requires keepalive (`eth_chainId` every 30s).
- **Debugging:** `mega-evme` CLI for transaction replay and opcode-level gas profiling (https://github.com/megaeth-labs/mega-evm)
- **Stage:** Not yet staged on L2Beat. Centralized sequencer, closed proofs (<5 external challengers), instant upgradability (no exit window).
...
| MegaETH  | Kumbaya                  | Uniswap V3 fork (concentrated liquidity)       | Native DEX, deepest MegaETH liquidity |
...
### Optimistic Rollups (Arbitrum, Optimism, Base, Unichain, Celo, MegaETH)
...
- **MegaETH:** Use `eth_sendRawTransactionSync` instead of `eth_sendRawTransaction` + polling. Hardcode gas limits (base fee is stable at 0.001 gwei). SSTORE to new slots is very expensive — use Solady's RedBlackTreeLib for slot reuse. Always use remote `eth_estimateGas` (MegaEVM costs differ from standard EVM).
...
| MegaETH  | https://rabbithole.megaeth.com/bridge | ~10-15 min | ~10.5 days  |
...
| MegaETH  | Testnet | 6343     | https://testnet.megaeth.com               |
